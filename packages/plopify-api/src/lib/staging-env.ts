...
export const stagingEnv = (template: string) => {
	 ...
	 */
	const setup = () => standardAdapter(async (input, output) => {
		...
		if (!exists) {
			let message;
			if (_templateType === 'local') {
				message = `Unable to locate "${_templateDir}".  Please check that the template path is accurate.`;
			} else if (_templateType === 'remote') {
				message = `The url, "${template}", either is not a valid git repository or the given credentials are incorrect.  Please check the url and credentials.`;
			}
		...
		if (_templateType === 'remote') {
			...
			if (result.status !== 0) {
				throw new Error('There was a problem cloning the template repository:\n' + result.stderr);
			}
		}

		// 5. Make sure a proper plopify config exists in the templateDir
		if (!fs.existsSync(path.resolve(_templateDir, '.plopifyrc.js'))) {
			throw new Error('The given template has no `.plopifyrc.js` file.  Please inspect your template.');
		}

		// If we make it here, we have succeeded
		output({ type: 'info', message: 'Staging environment created.' });
	});
