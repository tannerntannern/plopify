 ...
 */
export const init = (dir: string) => standardAdapter(async (input, output) => {
	...
	output({ type: 'info', message: `+${totalFiles} added at ${outDir}` });

	return { files: totalFiles, where: outDir };
});
